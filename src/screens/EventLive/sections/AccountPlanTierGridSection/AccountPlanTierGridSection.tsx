...
export const AccountPlanTierGridSection = ({
  plans,
  ...
  return (
    <div className="flex w-full items-center gap-2.5 pl-2 pr-0 py-8">
      {plans.map((plan) => {
        ...
        const bottomBorderClass = isPopular ? "border-[#00ffa333]" : "border-[#ffffff0d]";
        const bottomBgClass = isPopular ? "bg-[#0b0f1499]" : "bg-[#0b0f1466]";
        ...
        return (
          ...
          >
            <div
              className={`relative self-stretch w-full h-[365px] bg-[#0b0f14] rounded-xl border border-solid ${cardBorderClass} ${cardShadowClass} backdrop-blur-[6px] backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(6px)_brightness(100%)]`}
            >
              {isPopular && (
              ...
              <div className="flex flex-col w-[calc(100%_-_50px)] items-start absolute top-[25px] left-[25px]">
                <span
                  className={`[font-family:'Public_Sans',Helvetica] font-bold ${nameColor} text-xs tracking-[1.20px] leading-4 whitespace-nowrap`}
                >
            ...
            <div
              className={`${bottomBgClass} ${bottomBorderClass} flex flex-col items-start gap-4 p-4 self-stretch w-full rounded-xl border border-solid backdrop-blur-[6px] backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(6px)_brightness(100%)]`}
            >
              <div className="flex items-center justify-between self-stretch w-full">
                <span
                  className={`[font-family:'Public_Sans',Helvetica] font-bold ${nameColor} text-[10px] tracking-[0] leading-[15px] whitespace-nowrap`}
                >
                  {plan.name.toUpperCase()}
                </span>
