...
const MobileStep1 = ({
  ...
  <div className="flex flex-col gap-4 md:gap-6 lg:gap-8 flex-1">
    <div>
      <h1 className="[font-family:'Public_Sans',Helvetica] text-2xl md:text-3xl lg:text-4xl font-black tracking-tight">{t("checkout.title")}</h1>
      <div className="mt-1 md:mt-2 flex items-center gap-1.5 text-xs md:text-sm">
        <Link to="/challenge" className="text-gray-500">{t("checkout.breadcrumb_new")}</Link>
        <span className="text-gray-600">›</span>
        <span className="text-[#00FFA3]">{t("checkout.breadcrumb_confirm")}</span>
      </div>
    </div>
    ...
    <div className={`${gradientCardClass} flex flex-col gap-5 md:gap-7 lg:gap-8 w-full p-4 md:p-6 lg:p-8`}>
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-5 lg:gap-6">
        <div className="flex flex-col gap-1.5 md:gap-2">
          <label className="[font-family:'Public_Sans',Helvetica] font-bold text-gray-500 text-[9px] md:text-xs tracking-[1px] leading-3 md:leading-4">{t("checkout.full_name")}</label>
        ...
        <div className="flex flex-col gap-1.5 md:gap-2">
          <label className="[font-family:'Public_Sans',Helvetica] font-bold text-gray-500 text-[9px] md:text-xs tracking-[1px] leading-3 md:leading-4">{t("checkout.email")}</label>
          <div className="flex items-center h-11 md:h-14 lg:h-16 bg-[#0b0f14] rounded-lg border border-[#00ffa333] px-3 md:px-4">
        ...
        <div className="flex flex-col gap-1.5 md:gap-2">
          <label className="[font-family:'Public_Sans',Helvetica] font-bold text-gray-500 text-[9px] md:text-xs tracking-[1px] leading-3 md:leading-4">{t("checkout.country")}</label>
          <div className="flex items-center h-11 md:h-14 lg:h-16 bg-[#0b0f14] rounded-lg border border-[#00ffa333] px-3 md:px-4">
            <input
              value={country}
        ...
        <div className="flex flex-col gap-1.5 md:gap-2">
          <label className="[font-family:'Public_Sans',Helvetica] font-bold text-gray-500 text-[9px] md:text-xs tracking-[1px] leading-3 md:leading-4">{t("checkout.city")}</label>
          <div className="flex items-center h-11 md:h-14 lg:h-16 bg-[#0b0f14] rounded-lg border border-[#00ffa333] px-3 md:px-4">
            <input
              value={city}
    ...
    <div className={`${gradientCardClass} flex flex-col gap-4 md:gap-6 w-full p-4 md:p-6 lg:p-8`}>
      ...
      <div className="relative flex items-center gap-3 md:gap-4 h-[72px] md:h-[90px] lg:h-[100px] bg-[#2cf6c30d] rounded-xl border-none p-4 md:p-5 before:content-[''] before:absolute before:inset-0 before:p-[1px] before:rounded-xl before:[background:linear-gradient(227deg,rgba(44,246,195,0.3)_0%,rgba(1,50,38,0.3)_100%)] before:[-webkit-mask:linear-gradient(#fff_0_0)_content-box,linear-gradient(#fff_0_0)] before:[-webkit-mask-composite:xor] before:[mask-composite:exclude] before:z-[1] before:pointer-events-none">
        ...
        <div className="flex-1 min-w-0">
          <p className="[font-family:'Public_Sans',Helvetica] font-bold text-white text-xs md:text-sm">{t("checkout.crypto_payment")}</p>
          <p className="[font-family:'Public_Sans',Helvetica] text-[10px] md:text-xs text-slate-500">{t("checkout.crypto_sub")}</p>
        </div>
    ...
    <div className="flex items-center gap-3 md:gap-4 rounded-xl border border-[#1e3a42]/40 bg-[#050f15]/50 p-3 md:p-5 lg:p-6">
      ...
      <div>
        <p className="[font-family:'Public_Sans',Helvetica] text-[11px] md:text-sm font-bold text-white">{t("checkout.ssl_title")}</p>
        <p className="[font-family:'Public_Sans',Helvetica] text-[9px] md:text-xs text-gray-500 leading-relaxed">{t("checkout.ssl_sub")}</p>
      </div>
    </div>
...
const MobileStep2 = ({
  ...
  <div className="flex flex-col gap-4 md:gap-6 lg:gap-8 flex-1">
    <div className={`${gradientCardClass} flex flex-col gap-6 md:gap-8 lg:gap-10 w-full p-5 md:p-8 lg:p-10`}>
      <h2 className="[font-family:'Public_Sans',Helvetica] font-bold text-white text-xl md:text-2xl lg:text-3xl tracking-[-0.5px]">{t("checkout.your_order")}</h2>
      ...
      <div className="rounded-lg border border-[#ffffff0d] bg-[#02061780] px-4 md:px-6 py-4 md:py-5">
        <p className="[font-family:'Public_Sans',Helvetica] text-center text-[10px] md:text-xs leading-[1.6] text-gray-500">
          ...
          <span className="text-[#2cf6c3] cursor-pointer">{t("checkout.privacy_policy")}</span>. {t("checkout.terms_text")}
        </p>
      </div>
    </div>
      ...
      <div className="flex flex-col flex-1 relative">
        <header className="flex flex-col items-start gap-2 px-8 pt-[36px] pb-2 bg-transparent">
          ...
          <div className="flex flex-row items-center gap-0 h-5">
            <span className="[font-family:'Public_Sans',Helvetica] font-normal text-[#00ffa3] text-sm tracking-[0] leading-5 whitespace-nowrap">{t("checkout.breadcrumb_new")}</span>
            <img className="w-5 h-14 mx-[8px]" alt="Container" src="/svg/arrow-right-another.svg" />
            <span className="[font-family:'Public_Sans',Helvetica] font-normal text-gray-500 text-sm tracking-[0] leading-5 whitespace-nowrap">{t("checkout.breadcrumb_confirm")}</span>
          </div>
        </header>
      ...
      {redirectNotice ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/75 p-4">
          <div className="w-full max-w-lg rounded-3xl border border-[#163e4a]/60 bg-[#08141c] p-6 text-center shadow-2xl">
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#00FFA3]">{t("checkout.payment_created")}</p>
            <h2 className="mt-3 text-2xl font-black text-white">{t("checkout.thank_you")}</h2>
            <p className="mt-3 text-sm text-gray-300">
              {redirectNotice.challengeName} {t("checkout.redirect_text")}
            </p>
            <div className="mt-5 rounded-2xl border border-[#163e4a]/60 bg-[#05070a] px-4 py-3">
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gray-500">{t("checkout.nowpayments_id")}</p>
      ...
      <div className="xl:hidden min-h-screen bg-[#05070A] font-['Inter',sans-serif] text-white overflow-x-hidden flex flex-col">
        <header className="sticky top-0 z-50 flex h-14 items-center justify-between border-b border-[#2cf6c3]/30 bg-[#05070A]/95 px-4 backdrop-blur-md">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/challenge" className="rounded-lg bg-[#00FFA3] px-3 py-1.5 text-[11px] font-bold text-black">{t("trading.start")}</Link>
            ...
            {mobileNavTabs.map((tab) => {
              ...
              return (
                <Link key={tab.route} to={tab.route} onClick={() => setSidebarOpen(false)} className={`flex w-[210px] items-center gap-1.5 py-2 rounded-xl transition-all duration-200 ${isActive ? "bg-[#01ffa3] pl-[30px] pr-4" : "bg-transparent hover:bg-white/5 px-4"}`}>
                  <span className={`[font-family:'Inter',Helvetica] text-[13.2px] tracking-[0] leading-5 whitespace-nowrap transition-colors ${isActive ? "font-semibold text-[#05070a]" : "font-normal text-gray-300"}`}>{t(tab.labelKey)}</span>
                </Link>
              );
            })}
          </div>
